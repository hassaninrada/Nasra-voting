...
async function loadCandidates() {
    ...
    data.forEach((cand) => {
        ...
        card.innerHTML = `
                <div class="absolute top-0 right-0 w-20 h-20 bg-blue-50 rounded-bl-[3rem] flex items-center justify-center -mr-2 -mt-2 transition-all group-hover:bg-blue-900">
                ...
                <div class="w-32 h-32 mx-auto mb-6 relative">
                  <img src="${cand.image_url || 'https://via.placeholder.com/150'}"
                    class="w-full h-full object-cover rounded-full border-4 border-white shadow-xl group-hover:border-blue-900 transition-colors" />
                </div>
                ...
                <button
                  class="w-full bg-blue-100/50 text-blue-900 font-black py-4 rounded-2xl group-hover:bg-blue-900 group-hover:text-white transition-all uppercase text-[10px] tracking-widest">Lock Voting Choice</button>
            `;
...
function castVote(candidate) {
    localStorage.setItem("vote_deputyboy_secondary", candidate);
    document.body.style.opacity = "0";
    document.body.style.transition = "opacity 0.4s ease";
    setTimeout(() => { window.location.href = "deputygirl-secondary.html"; }, 600);
}
