...
// APP STATE
const roleNames = {
    headboy_secondary: "Wing Head Boy",
    headgirl_secondary: "Wing Head Girl",
    deputyboy_secondary: "Deputy Head Boy",
    deputygirl_secondary: "Deputy Head Girl"
};
...
window.fetchCandidates = async () => {
    ...
    const { data, error } = await supabase.from('candidates').select('*').eq('wing', 'Secondary');
    if (error) {
...
// REALTIME
supabase.channel('dashboard-secondary')
...
document.getElementById("resetAllBtn")?.addEventListener("click", async () => {
    if (confirm("⚠️ CAUTION: Are you sure you want to reset ONLY SECONDARY votes? Primary wing votes will stay safe.")) {
        try {
            const secondaryRoles = ['headboy_secondary', 'headgirl_secondary', 'deputyboy_secondary', 'deputygirl_secondary'];
            await supabase.from('vote_counts').delete().in('role', secondaryRoles);
            await supabase.from('votes_audit').delete().in('role', secondaryRoles);
            await supabase.from('students').update({ voted_secondary: false }).eq('wing', 'Secondary');
            location.reload();
...
document.getElementById("exportBtn")?.addEventListener("click", async () => {
    const secondaryRoles = ['headboy_secondary', 'headgirl_secondary', 'deputyboy_secondary', 'deputygirl_secondary'];
    const { data } = await supabase.from('vote_counts').select('*').in('role', secondaryRoles);
    if (!data) return;
    ...
    a.setAttribute('download', 'SECONDARY_VOTING_EXPORT.csv');
